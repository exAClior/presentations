...
* **Double-edged Sword**: "Circuit Extraction is #P-hard" [^CircExtra]
"""
...
- **Transversal gates**: "a logical gate that can be implemented on a logical qubit by the independent action of separate physical gates on corresponding physical qubits"[^EKWiki]. $(RobustLocalResource(urlintro, pathftH,:width=>400, :align=>:center, :alt=>"FT H", cache=false)) $(RobustLocalResource(urlintro, pathftcx,:width=>400, :align=>:center, :alt=>"FT H", cache=false)) [^TheBible]

- **A Universal Gateset**: Clifford + T
- **Clifford gates**: generated by CNOT, H, S which are implementable as transversal gates by error correction codes such as the Steane code [^TheBible].
- **T Gate**: can be implemented using costly magic state distillation then magic state injection.[^MagicCosts]
- **Magic State Distillation**: "a magic state distillation procedure yields one copy of a magic state (with any desired fidelity) from several copies of the state ρ, provided that the initial fidelity between ρ and the magic state to be distilled is large enough." [^BKDistill]
"""
