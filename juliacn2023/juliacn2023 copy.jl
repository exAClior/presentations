...
begin
	...
	pathmultbtrig = "figs/multiWSpd.png"
...
## ZX-calculus
- **Calculus**: "连续变化的数学研究" [^fn1]
- **演算**: "依照一定的原理、公式计算" [^fn2]
- **Example**: $\cos(\theta)^2 + \sin(\theta)^2 = 1$
- **Z 张量**: $(RobustLocalResource(urltp, pathzspbk,:width=>800,:align=>:center, :alt=>"Z Spider", cache=false))
- **X 张量**: $(RobustLocalResource(urltp, pathxspbk,:width=>800,:align=>:center, :alt=>"X Spider", cache=false))
"""
...
md"""## Power of ZX-calculus
* **Universality**：使用 X 和 Z 张量，我们可以表示 一切 ！
每一个量子电路

任意线性映射：2^n \rightarrow 2^m 也可以！[^LMUniv]

* "电路提取是 #P-hard" [^CircExtra]
"""
...
- **ZX-图**：一种多重图， 两个顶点之间可能存在多条边，其中顶点是 Z 和 X 张量。
- 目的是什么？
"""
...
- **Completeness**: "当图形演算的重写规则足够强大以证明任何真实等式时，该图形演算就是完备的。"[^ZXWorking]
- **说人话**: 如果两个 ZX-图 相等，上述重写规则足以将一个转化为另一个。
...
md"""## Applications
### Background 
- **Eastin–Knill theorem**: "没有任何量子纠错码能够检测到所有的局部错误并且允许一套普适的 transversal gate"[^StackExchange].
- **Transversal gates**: "一个逻辑门，可以通过对相应的物理量子比特执行独立的物理门来在逻辑量子比特上实现"[^EKWiki]. $(RobustLocalResource(urlintro, pathftH,:width=>275, :align=>:center, :alt=>"FT H", cache=false)) $(RobustLocalResource(urlintro, pathftcx,:width=>275, :align=>:center, :alt=>"FT H", cache=false)) [^TheBible]

- **A Universal Gateset**: Clifford + T
- **Clifford gates**: 由CNOT、H、S生成，这些可以通过像Steane编码这样的纠错码作为横向门来实现。[^TheBible]
- **T Gate**: 可以通过昂贵的魔法状态提纯然后进行魔法状态注入来实现。[^MagicCosts]
- **Magic State Distillation**: "一个魔法状态提纯程序可以从多个ρ状态的副本中产生一个魔法状态的副本（具有任何所需的保真度），只要ρ与要提纯的魔法状态之间的初始保真度足够大。" [^BKDistill]
"""
...
- **Recall**: T gates are costly
- **Hadamard Edge**: $(RobustLocalResource(urlintro, pathhadaedg,:width=>400, :align=>:center, :alt=>"Translation table", cache=false))
- **Phase Gadget**: $(RobustLocalResource(urlintro, pathphgad,:width=>150, :align=>:center, :alt=>"Translation table", cache=false))
...
# ╔═╡ 89fc3a74-75f4-4edf-89a1-5b33df039f7b
tcount(zxd)
...
# ╔═╡ 3ba3f3c9-7c51-40a2-94ff-939ef738fcd6
tcount(zxd_reduced)
...
- **More Legs**:  $(RobustLocalResource(urlintro, pathmultbtrig,:width=>400, :align=>:center, :alt=>"W Spider", cache=false))
- **Braket Notation**:  $(RobustLocalResource(urlintro, pathbtrig,:width=>400, :align=>:center, :alt=>"W Spider", cache=false))
...
- **Parameterized Quantum Circuit**: training of which is the goal of hybrid quantum classical algorithms like quantum chemistry or quantum machine learning.
- **Barren Plateau**: "the gradient (of expectation value for a Parameterized Quantum Circuit) vanishes exponentially as the system grows up" [^BarrenPlateau]
- **Expectation value**: $\langle H \rangle = \langle 0 | U^{\dagger}(\vec{\theta}) H U(\vec{\theta}) |0\rangle$
- **Variance of Expectation value**: [^DiffInt]
```math
\begin{align*}
...
begin
	...
	vizcircuit(zxwd; fontsize = 15, bgcolor = "white", linewidth = 5, circsize = 14, vrot = 0.0, graphwidth = 20, graphheight = 6)
end;

# ╔═╡ b4fb8daa-4906-4b2f-9951-0a629f03478b
begin
	exp_zxwd = expval_circ!(zxwd, "ZZ")
	vizcircuit(exp_zxwd; fontsize = 5, bgcolor = "white", linewidth = 5, circsize = 10, vrot = 0.0, graphwidth = 30, graphheight = 12)
end;
...
# ╔═╡ 8d12489e-1b4d-4f51-a8dc-152abb19a962
md"""## W Spider and Summation of Linear Maps
